...
function rngFromSeed(seed){
  const x = xmur3(seed)();
  return sfc32(x, x^0x9E3779B1, x^0x85EBCA77, x^0xC2B2AE3D);
}
...
const SHADOW = (hex, k=0.82) => {
  const n = parseInt(hex.slice(1), 16);
  const r = n >> 16;
  const g = n >> 8 & 255;
  const b = n & 255;
  const d = v => Math.max(0, Math.min(255, Math.round(v * k)));
...
function drawHead(ctx, {skin='F3', eyeColor='#3a2f25', hair='short', hairColor='#2b2018', brows=true, facial='none', accessory='none', accent='#EA6A11'}){
  ...
  const SK = SKIN[skin] || SKIN.F3;

  // neck
  el(ctx, 64, 99, 16, 11);
  ctx.fillStyle = SK;
  ctx.fill();
  ctx.strokeStyle = SHADOW(SK, 0.7);
  ctx.lineWidth = 1;
  ctx.stroke();
  ...
  // jaw shadow
  el(ctx, 64, 80, 22, 12);
  ctx.fillStyle = SHADOW(SK, 0.92);
  ctx.globalAlpha = 0.35;
  ctx.fill();
  ctx.globalAlpha = 1;

  // eyes
  el(ctx, 53, 64, 6.8, 6.8);
  ctx.fillStyle = '#fff';
  ctx.fill();
  el(ctx, 75, 64, 6.8, 6.8);
  ctx.fillStyle = '#fff';
  ctx.fill();
  el(ctx, 53, 65, 3.7, 3.7);
  ctx.fillStyle = eyeColor;
  ctx.fill();
  el(ctx, 75, 65, 3.7, 3.7);
  ctx.fillStyle = eyeColor;
  ctx.fill();
  el(ctx, 55, 63.5, 1.6, 1.6);
  ctx.fillStyle = '#fff';
  ctx.fill();
  el(ctx, 77, 63.5, 1.6, 1.6);
  ctx.fillStyle = '#fff';
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.moveTo(60, 76);
  ctx.quadraticCurveTo(64, 78, 68, 76);
  ctx.stroke();

  // mouth (neutral)
  ctx.strokeStyle = SHADOW(SK, 0.65);
  ctx.lineWidth = 1.8;
  ctx.beginPath();
  ctx.moveTo(56, 86);
  ctx.quadraticCurveTo(64, 89, 72, 86);
  ctx.stroke();
  ...
  switch(hair){
    ...
    case 'waves':
      hairBase();
      ctx.strokeStyle = SHADOW(hairColor, 0.7);
      ctx.lineWidth = 1;
      for(let y = 40; y < 56; y += 4){
        ctx.beginPath();
        ctx.moveTo(40, y);
        ctx.bezierCurveTo(54, y + 2, 74, y - 2, 88, y + 2);
        ctx.stroke();
      }
      break;
    case 'afro':
    ...
    case 'curls':
      ctx.fillStyle = hairColor;
      for(let x = 40; x <= 88; x += 8){
        el(ctx, x, 44 + ((x / 8) % 2 ? 2 : 0), 5.2, 5.2);
        ctx.fill();
      }
      el(ctx, 37, 58, 7, 15);
      ctx.fill();
    ...
    case 'beanie':
      ctx.fillStyle = hairColor;
      rr(ctx, 36, 38, 56, 24, 10);
      ctx.fill();
      ctx.fillStyle = SHADOW(hairColor, 0.6);
      rr(ctx, 36, 56, 56, 8, 6);
      ctx.fill();
      break;
    case 'durag':
      hairBase();
      ctx.fillStyle = hairColor;
      rr(ctx, 36, 38, 56, 20, 10);
      ctx.fill();
      rr(ctx, 84, 58, 10, 22, 5);
      ctx.fill();
      break;
    case 'headband':
      hairBase();
      ctx.fillStyle = '#fff';
      rr(ctx, 38, 46, 52, 10, 5);
      ctx.fill();
...
export function dnaFromSeed(seed){
  return randomDNA(seed);
}